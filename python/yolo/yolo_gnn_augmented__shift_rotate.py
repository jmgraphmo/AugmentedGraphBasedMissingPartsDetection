...
class BikePartsDetectionDataset(Dataset):
    ...
    def apply_augmentation(self, image, boxes, labels=None):
        ...
        if augment <1/2: 
            ...
            # failsafe
            # clamp boxes back if shift puts them outside valid range, only keep boxes if it is still valid 
...
if not skip_yolo_training:
    yolo.train(
        ...
        name="bikeparts_gnn_augmented_shift_rotate",
        exist_ok=True,
    )
...
for epoch in range(1, num_epochs + 1):
    if True:
        ...
        if macro_f1 > best_macro_f1:
            ...
            torch.save(
                gnn.state_dict(),
                "models/yolo/yolo_gnn_augmented_model_shift_rotate.pth",
            )
        else:
            no_improve += 1
    ...
    table = [
        ...
        #["COÃ¢â€šâ€š Emissions (kg)", f"{co2_emissions:.4f} kg"],
    ]
