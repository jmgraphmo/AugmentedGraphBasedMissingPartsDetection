                ...
                outputs.append(
                    {
                        ...
                        "scores": torch.zeros((0,), device=img.device),
...
sched_det = ReduceLROnPlateau(
    opt_det, mode="max", factor=0.5, patience=3, min_lr=1e-6
)

opt_graph = torch.optim.AdamW(graph_params, lr=1e-4, weight_decay=1e-4)

sched_graph = ReduceLROnPlateau(
    opt_graph, mode="max", factor=0.5, patience=3, min_lr=1e-6
)
...
for epoch in range(1, epochs + 1):
    if epoch < freeze_epoch:
        ...
        if macro_f1 > detector_best_macro_f1:
            ...
            torch.save(
                ...
                "models/graph_rcnn/graphrcnn_detector_baseline_model2.pth",
            )
        else:
    ...
    model.detector.load_state_dict(
        torch.load(
            "models/graph_rcnn/graphrcnn_detector_baseline_model.pth"
        )
    )
    ...
    if macro_f1 > joint_best_macro_f1:
        ...
        torch.save(
            model.state_dict(),
            f"models/graph_rcnn/graphrcnn_MobileNet_baseline_model.pth",
        )
    else:
        joint_no_improve += 1
...
model.load_state_dict(
    torch.load(
        "models/graph_rcnn/graphrcnn_MobileNet_baseline_model.pth",
        map_location=device,
    )
)
model.to(device)
