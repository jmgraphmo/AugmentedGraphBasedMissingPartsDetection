...
for epoch in range(1, epochs + 1):
    ...
    if epoch < freeze_epoch:
        ...
        if macro_f1 > detector_best_macro_f1:
            ...
            torch.save(
                ...
                "models/graph_rcnn/graphrcnn_detector_augmented_model_shift.pth",
            )
        else:
    ...
    if macro_f1 > joint_best_macro_f1:
        ...
        torch.save(
            model.state_dict(),
            f"models/graph_rcnn/graphrcnn_MobileNet_augmented_model_shift.pth",
        )
    else:
        joint_no_improve += 1
...
model.load_state_dict(
    torch.load(
        "models/graph_rcnn/graphrcnn_MobileNet_augmented_model_shift.pth",
        map_location=device,
    )
)
model.to(device)
